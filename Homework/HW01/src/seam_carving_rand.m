...
path_write = "../data/" + pic_name + "_result_rand." + pic_type;
im = imread(path_read);
...
function seam = find_vertical_seam(G)
    ...
    % 改进：从纵坐标中随机选择位置开始，而不是只从最后一行
    start_row = randi([1, rows]);
    [~, start_col] = min(M(rows,:));

    % 反向追踪最优接缝
    seam(start_row) = start_col;
    for r = start_row-1:-1:1
        c = seam(r+1);
        if c == 1
